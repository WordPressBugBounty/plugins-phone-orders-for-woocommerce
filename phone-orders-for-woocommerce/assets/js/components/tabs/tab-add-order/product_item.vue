	...
	<template v-else>
      ...
      @change="onChangeQtyAndMaybeFormat"
		...
		<template v-if="!isGiftItem">
			...
			<input type="text" autocomplete="off" placeholder="0" v-model.lazy="itemDiscountValue" :disabled="!cartEnabled || !allowDiscount" class="form-control">
		    </div>
		</template>
	    </td>
	    <td class="item_cost">
...
<script>
    ...
    export default {
	    ...
	    var itemDiscount = typeof this.item.wpo_item_discount !== 'undefined' ? this.item.wpo_item_discount : {discount: 0, original_price: this.item.item_cost, discounted_price: null, discount_type: 'fixed'};
