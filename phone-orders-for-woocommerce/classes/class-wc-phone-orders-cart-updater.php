...
class WC_Phone_Orders_Cart_Updater {
	...
	public function process( $cart_data ) {
		...
		remove_action( 'woocommerce_add_to_cart', array( WC()->cart, 'calculate_totals' ), 20 );
                ...
                if ( ! empty( $cart_data['dont_apply_pricing_rules'] ) || $this->option_handler->get_option( 'dont_apply_pricing_rules' )) {
		...
		if ( $this->option_handler->get_option( 'sale_backorder_product' ) ) {
			...
			add_filter( 'woocommerce_product_backorders_allowed', '__return_true' );
                ...
                add_filter( 'woocommerce_product_backorders_allowed', function($status, $product_id, $product) use($already_reduced_product_ids) {
		...
		$this->woocs_cmp = new WC_Phone_Woocs_Compatibility(isset($cart_data['order_currency']['code']) ? $cart_data['order_currency']['code'] : '');
...
//			}
			...
			if($cart_item_key AND $this->option_handler->get_option('allow_to_rename_cart_items') && !empty($item['custom_name'])) {
				WC()->cart->get_cart()[$cart_item_key]['data']->set_name($item['custom_name']);
			}

			if ( $cart_item_key ) {
				if ( apply_filters('wpo_update_cart_set_cart_item_data_price', wc_format_decimal((string)$item['item_cost'], wc_get_price_decimals()) != wc_format_decimal((string)$product->get_price(), wc_get_price_decimals()), $item, $cart_item_key, WC()->cart) ) {
					WC()->cart->get_cart()[ $cart_item_key ]['data']->set_price( apply_filters('wpo_update_cart_set_cart_item_data_set_price', $item['item_cost'], $item, $cart_item_key, WC()->cart) );
				}
...
//         (new WcCartPartialTotals(WC()->cart))->calculateTotalsWithoutShipping();
		...
		if ( isset( $cart_data['fee'] ) && is_array( $cart_data['fee'] ) ) {
			...
			add_action( 'woocommerce_cart_calculate_fees', function () use ( $fees_data, $tax_class, $cart_data_fee_ids, &$fee_ids, &$fee_cart_data ) {
				foreach ( $fees_data as $index => $fee_data ) {
					...
					if( !isset($fee_data['type'])) $fee_data['type'] = 'fixed';
        ...
        if ( ! empty( $cart_data['edit_order_id'] ) ) {
            ...
            $_coupons = method_exists($edit_order, 'get_coupon_codes') ? $edit_order->get_coupon_codes() : $edit_order->get_used_coupons();
            ...
            add_filter( 'woocommerce_coupon_validate_user_usage_limit', function($valid, $user_id, $coupon) use ($edit_order_coupons) {
                if (in_array($coupon->get_code(), $edit_order_coupons)) {
                    return false;
                }
                return $valid;
		...
		if ( ! empty( $cart_data['discount'] ) && ( $manual_cart_discount_code || $cart_data['discount']['name'] ) ) {
			...
			if ( empty( $discount['type'] ) ) {
				$discount['type'] = 'fixed_cart';
                        ...
                        add_filter('woocommerce_coupon_get_amount', function ($_discount, $coupon) use ($discount, $manual_cart_discount_code) {
...
//            } else {
			...
			if ( $this->custom_prod_control->is_custom_product( $product ) ) {
				...
				$loaded_products = $this->get_formatted_product_items_by_ids( array( array( 'id' => $product_id, 'qty' => $item['qty'], 'reduced_stock' => isset($item['reduced_stock']) ? $item['reduced_stock'] : null ) ) );
			}
			...
			if ( ! empty( $loaded_products ) ) {
                ...
                $item['loaded_product']['readonly_custom_meta_fields_html'] = wc_get_formatted_cart_item_data( $item );
...
//            }
	...
	public function get_item_by_product( $product, array $item_data = array() ) {
		...
		$default_qty_step = $this->option_handler->get_option( 'allow_to_input_fractional_qty' ) || apply_filters("wpo_allow_fractional_qty_for_product", false, $product ) ? '0.01' : '1';
        ...
        if( !$product->managing_stock() ) { //we know only status for the product
            ...
            if(isset($item_data['reduced_stock'])) //edit existing order ?
                $in_stock = $product->get_stock_quantity() > 0 ? $product->get_stock_quantity() + $item_data['reduced_stock'] : $item_data['reduced_stock'];
            else
