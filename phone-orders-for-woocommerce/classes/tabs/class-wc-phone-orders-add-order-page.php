	...
	public function render() {
		...
		$this->tab_data = array(
                    ...
                    'addDiscountModalSettings'   => array(
                        ...
                        'discountType'		    =>$this->option_handler->get_option( 'default_discount_type' ), // discount type by default
		...
		$tab_data = array(
			...
			'orderDetailsSettings'         => array(
				...
				'addProductFromShopTitle'                   => __('Browse shop and add products to cart', 'phone-orders-for-woocommerce'),
				...
				'barcodeModeAlertMessage'			=> __('Barcode mode enabled! Product search works only after pressing the Enter key', 'phone-orders-for-woocommerce'),
				...
				'dontApplyPricingRulesLabel'    => __('Don\'t apply pricing rules', 'phone-orders-for-woocommerce'),
				'pricingRulesEnabled'           => defined('WC_ADP_PLUGIN_FILE'),
				'productHistoryButtonLabel'     => __('Products history', 'phone-orders-for-woocommerce'),
				'productHistoryButtonDescription' => __('Select customer to see purchased products', 'phone-orders-for-woocommerce'),
			),
            ...
            'orderCurrencySelectorSettings' => array(
				'title'             => apply_filters( 'wpo_order_currency_selector_container_header', __( 'Order currency', 'phone-orders-for-woocommerce' ) ),
				'orderCurrenciesList' => apply_filters( 'wpo_order_currency_selector_list_currencies', $order_currencies_list),
                'orderDefaultCurrency' => array('code' => get_option('woocommerce_currency'), 'symbol' => get_woocommerce_currency_symbol( get_option('woocommerce_currency') )),
				'noOptionsTitle'    => __( 'List is empty.', 'phone-orders-for-woocommerce' ),
			),
		...
		?>
                ...
                <tab-add-order v-bind="<?php echo esc_attr( json_encode( $this->tab_data ) ) ?>">
                    ...
                    <template v-slot:find-or-create-customer>
                        <find-or-create-customer v-bind="<?php echo esc_attr(json_encode($tab_data['findOrCreateCustomerSettings'])) ?>" ref="findOrCreateCustomer">
                            <?php do_action( 'wpo_after_customer_details' ) ?>
                        </find-or-create-customer>
                    </template>
                    <template v-slot:order-details>
                        <order-details v-bind="<?php echo esc_attr(json_encode($tab_data['orderDetailsSettings'])) ?>">
                ...
                <?php do_action('wpo_order_footer_left_side') ?>
                    ...
                    <template v-slot:order-date>
                        <order-date v-bind="<?php echo esc_attr(json_encode($tab_data['orderDateSettings'])) ?>" ></order-date>
                    </template>
                    <template v-slot:order-status>
                        <order-status v-bind="<?php echo esc_attr(json_encode($tab_data['orderStatusSettings'])) ?>" ></order-status>
                    </template>
                    <template v-slot:order-currency-selector>
                        <order-currency-selector v-bind="<?php echo esc_attr(json_encode($tab_data['orderCurrencySelectorSettings'])) ?>" ></order-currency-selector>
                    </template>
                    <template v-slot:order-payment-method>
                        <order-payment-method v-bind="<?php echo esc_attr(json_encode($tab_data['orderPaymentMethod'])) ?>" ref="orderPaymentMethod" ></order-payment-method>
                    </template>
                </tab-add-order>
		<?php
	}
...
//		$item_subtotal = ( isset( $item['line_subtotal'] ) ) ? esc_attr( wc_format_localized_price( $item['line_subtotal'] ) ) : '';
	...
	protected function get_formatted_address( $customer_data ) {
		...
		$billing_address = htmlspecialchars_decode(WC()->countries->get_formatted_address( apply_filters('wpo_customer_formatted_address', $billing_data, $customer_data, 'billing') ));
		$shipping_address = htmlspecialchars_decode(WC()->countries->get_formatted_address( apply_filters('wpo_customer_formatted_address', $shipping_data, $customer_data, 'shipping') ));
	...
	protected function create_order( $data, $set_status = true ) {
		...
		if ( ! empty( $data['order_status'] ) && $set_status ) {
			...
			if ( $new_status AND $new_status != $order->get_status() ) {
				...
				$_new_status = wc_is_order_status( 'wc-' . $order->get_status() ) ? 'wc-' . str_replace('wc-', '', $new_status) : $new_status;
		...
		if( apply_filters("wpo_add_note_created_with", true) ) {
			...
			$note_text = sprintf( __( 'Created in Phone Orders by: %s.', 'phone-orders-for-woocommerce' ), $admin_user->display_name );
...
//				}
	...
	public function action_woocommerce_checkout_create_order_shipping_item( $item, $package_key, $package, $order ) {
		...
		$item->add_meta_data( WC_Phone_Orders_Cart_Shipping_Processor::ORDER_SHIPPING_METHOD_ID_KEY, $shipping_rate->get_id() );
	}
    ...
    protected function ajax_create_order_email_invoice( $data ) {
		...
		try {
			...
			if( apply_filters("wpo_add_system_note_invoice_sent", false) )
				$order->add_order_note( __( 'Invoice manually sent to customer from Phone Order.', 'phone-orders-for-woocommerce' ) );
			else
	...
	protected function ajax_search_products_and_variations( $data ) {
		...
		$result = array();
		$delimiter = apply_filters( 'wpo_autocomplete_product_fields_delimiter', $this->option_handler->get_option( 'display_search_result_as_grid' )  ? '<br/>' : '|' );
        ...
        add_filter( 'woocommerce_currency_symbol', function ($currency_symbol, $currency) use ($data) {
            return isset($data['cart']['order_currency']['code']) && $this->option_handler->get_option('show_order_currency_selector') ? $data['cart']['order_currency']['symbol'] : $currency_symbol;
        }, 10, 2);
		...
		foreach ( $products as $product_id => $product) {
			...
			$rev_points = 0;
			if($term AND stripos($product->get_name(),$term) !== false) $rev_points += 2;
			if($term AND stripos($product->get_description(),$term) !== false) $rev_points += 1;
	...
	if ( $customer_id ) {
        ...
        $state = array(
            ...
            'default_order_currency' => array('code' => get_option('woocommerce_currency'), 'symbol' => get_woocommerce_currency_symbol( get_option('woocommerce_currency') )),
    ...
    protected function load_order( $order_id, $mode = '' ) {
		...
		foreach ( $order->get_items() as $key => $order_item ) {
			...
			if ( ! $_product->is_in_stock() AND ! $option_handler->get_option( 'sale_backorder_product' ) && $mode !== 'view' && $mode !== 'edit' ) {
			...
			$loaded_product['custom_name'] = $item_data['name'];
                        ...
                        if ( $mode === 'edit' ) {
                            ...
                            } else if( ! $_product->is_in_stock() &&  ! $_product->managing_stock() ) {
		...
		// private note
		$cart['private_note'] = apply_filters('wpo_load_order_private_note', $order->get_meta( $this->meta_key_private_note, true ), $order_id, $order);
        ...
        $cart['order_currency'] = array('code' => $order->get_currency(), 'symbol' => get_woocommerce_currency_symbol( $order->get_currency() ));
